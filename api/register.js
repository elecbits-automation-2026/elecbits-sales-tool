...
//
// Env (same service account as /api/drive):
...
async function appendVerified(token, sid, tab, idValue, rowValues) {
  ...
  await append();
  let row = await find();
  if (!row) { await append(); row = await find(); }  // the one repair attempt
...
// counts: renames of the human half must not orphan the deals inside.
async function findClientFolder(token, root, clientId, clientName) {
  ...
  const l = await gdrive(token, "files", {
    q: `name contains '${esc(clientId)}' and mimeType = '${FOLDER_MIME}' and trashed = false`,
    fields: "files(id,name,webViewLink)", pageSize: "5",
    supportsAllDrives: "true", includeItemsFromAllDrives: "true",
  });
  return (l.files || []).find((f) => f.name.startsWith(clientId)) || null;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "peek") {
      ...
      let max = 0;
      for (const v of await readCol(token, reg.id, "Clients", "A")) {
        const m = v.match(re);
    ...
    if (action === "client") {
      ...
      if (reg) {
        try {
          duplicate = (await readCol(token, reg.id, "Clients", "A")).includes(clientId);
          if (!duplicate) {
            row = await appendVerified(token, reg.id, "Clients", clientId, [
              clientId, S(b.legacyId), name, S(b.sector), S(b.orgSize),
              S(b.status) || "Active", "", new Date().toISOString().slice(0, 10),
              S(b.addedBy), S(b.poc), S(b.notes),
            ]);
            registered = true;
          }
        } catch (e) { warnings.push("register: " + String(e.message || e)); }
      } else {
    ...
    if (action === "deal") {
      ...
      if (reg) {
        try {
          duplicate = (await readCol(token, reg.id, "Deals", "A")).includes(dealId);
          if (!duplicate) {
            row = await appendVerified(token, reg.id, "Deals", dealId, [
              dealId, clientId, S(b.dealName), S(b.status) || "Open",
              S(b.value), S(b.currency) || "INR", S(b.owner),
              S(b.dateOpened) || new Date().toISOString().slice(0, 10),
              "", "", "", "", S(b.notes),
            ]);
            registered = true;
          }
