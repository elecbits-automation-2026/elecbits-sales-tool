...
## The process map Sales does not have

The PMS anchors every task to one of **308 process steps**, generated by
`scripts/build-process-map.mjs` from five documents in `docs/process/`. A task
that knows its step opens with that step's action, its entry and exit
questions, its template, and the exact Drive path its output is filed at.

Sales has ten pipeline stages and no documents behind them, so `src/data/
stages.ts` carries a `STAGE_GUIDE` at the same shape. Read the provenance
comment there before trusting it:

| Field | Where it comes from |
|---|---|
| exit questions | `DEFAULT_GATES` — **real**, the list the stage gate already interviews against |
| entry questions | derived: the previous stage's exit questions |
| action / whatToDo / produces / guidance | **a first draft**, inferred from what the gates demand. Nobody in sales wrote them. |

Treat a wrong line in that last row as a bug report against the file, not as
the method. When these start changing often, generate the file from whatever
the sales team maintains, the way the PMS does.

`TaskChat` is likewise the readable half of the PMS's `WorkChat`: it knows the
task, account, deal and stage and can read the company's Drive folder through
`askWithDrive`, but it cannot **edit** a Drive document the way the PMS can —
that depends on machinery Sales has no equivalent of.
